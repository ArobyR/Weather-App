...
async function getInformation() {
    try {
        let cityInputValue = cityInput.value;
        let countryInputValue = countryInput.value;
        let request = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityInputValue},${countryInputValue}&appid=2681280f473ded888f58ef7409d47db5&units=metric`);
...
async function showInformation() {
    ...
    if (information == undefined) cityTitle.textContent = 'Error, check your connection' + "!";
        ...
        weatherTemperature.textContent = (information.main.temp).toPrecision(2) + "°C";
